...
export default function MenuCustomizeModal({ item, onClose, onAdd }) {
  ...
  const extrasOptions = [
    ...
    { name: "Gluten-Free Option", price: 1.5 },
  ];
  ...
  const extraCost = selectedExtras.length * 1.5;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-2 sm:p-4 overflow-y-auto">
      <div className="relative w-full max-w-sm sm:max-w-lg bg-white border-none rounded-3xl sm:rounded-4xl shadow-soft overflow-hidden animate-in fade-in zoom-in-95 duration-200 flex flex-col max-h-[90vh]">
        {/* Header Photo */}
            ...
            <div className="bg-slate-50 border border-slate-200 rounded-xl p-4">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-center mb-3">
                ...
                <div className="p-2 bg-white rounded-lg border border-slate-200 shadow-sm">
                  ...
                  <p className="font-bold text-slate-900 text-sm">
                    {item.nutritionalInfo.protein || "0g"}
                  </p>
                </div>
                <div className="p-2 bg-white rounded-lg border border-slate-200 shadow-sm">
                  <p className="text-[10px] text-slate-500 font-medium">
                  ...
                  <p className="font-bold text-slate-900 text-sm">
                    {item.nutritionalInfo.carbs || "0g"}
                  </p>
                </div>
                <div className="p-2 bg-white rounded-lg border border-slate-200 shadow-sm">
                  <p className="text-[10px] text-slate-500 font-medium">Fat</p>
                  <p className="font-bold text-slate-900 text-sm">
                    {item.nutritionalInfo.fat || "0g"}
                  </p>
                </div>
              </div>

              {item.nutritionalInfo.allergens &&
        ...
        <div className="p-6 bg-white border-t border-slate-100 shrink-0 flex items-center justify-between gap-4">
          ...
          >
            ...
            <span className="font-black text-white/90 border-l border-white/20 pl-3 ml-2">
              ${itemTotal.toFixed(2)}
            </span>
          </button>
        </div>
