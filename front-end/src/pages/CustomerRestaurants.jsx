        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-8 mt-4">
          <div className="relative z-10 w-full md:w-1/2">
            <h1 className="text-4xl sm:text-5xl md:text-6xl font-black mb-4 text-[#1A1A1A] leading-tight">
              Welcome back,<br /><span className="text-brand-500">{user?.name?.split(' ')[0]}! 👋</span>
            </h1>
            <p className="text-slate-500 font-medium mb-8 text-lg pr-4">What are you craving today? Explore top restaurants and exclusive deals in your town.</p>
          ...
          <div className="space-y-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 pt-6">
              {offers.slice(0, 3).map(offer => (
                <div key={offer._id} onClick={() => navigate(`/user/restaurants/${offer.restaurant._id}`)} className="bg-white rounded-4xl p-6 shadow-soft hover:-translate-y-1 transition-all duration-300 cursor-pointer flex flex-col items-center text-center relative mt-6">
            ...
            {offers.length > 3 && (
               <div className="text-center mt-8">
                 <button onClick={() => navigate('/user/offers')} className="px-8 py-3 rounded-full bg-brand-50 text-brand-600 font-bold hover:bg-brand-500 hover:text-white transition-colors">See All Offers →</button>
               </div>
            )}
          </div>
        )}
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {restaurants.map(restaurant => {
                ...
                return (
                  <div key={restaurant._id} onClick={() => navigate(`/user/restaurants/${restaurant._id}`)} className={`bg-white rounded-3xl overflow-hidden border border-slate-100 shadow-sm hover:shadow-xl transition-all cursor-pointer group ${!available && 'opacity-75 grayscale-[0.5]'}`}>
