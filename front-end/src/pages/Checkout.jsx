...
export default function Checkout() {
  ...
  const defaultAddressId = user?.savedAddresses?.find(a => a.isDefault)?._id || (user?.savedAddresses?.length > 0 ? user.savedAddresses[0]._id : '');
  ...
  const [useSavedCard, setUseSavedCard] = useState(
    user?.savedPaymentMethods?.length > 0 ? user.savedPaymentMethods[0].token : ''
  );
  ...
  const handlePlaceOrderSubmit = async (e) => {
    ...
    try {
      ...
      if (useSavedAddress) {
        finalAddress = user.savedAddresses.find(a => a._id === useSavedAddress);
      }

      if (!finalAddress) {
         addNotification('Please select or provide a valid delivery address.', 'error');
         setSubmitting(false);
         return;
      }
      ...
      const res = await fetch(`${API_BASE_URL}/orders`, {
        ...
        body: JSON.stringify({
          ...
          paymentMethod: paymentMethodType === 'cod' ? 'cod' : (useSavedCard ? 'saved_card' : 'card'),
          deliveryType,
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      <h1 className="text-2xl sm:text-4xl font-black text-[#1A1A1A] text-left">Checkout</h1>
      ...
      <form onSubmit={handlePlaceOrderSubmit} className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
          ...
          <div className="bg-white border border-slate-200 rounded-3xl p-6 space-y-4 shadow-sm text-left">
            ...
            {user?.savedAddresses && user.savedAddresses.length > 0 ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {user.savedAddresses.map((addr) => (
                    ...
                    >
                      ...
                      <span className="text-xs font-bold">{addr.label || addr.fullName}</span>
              ...
              <div className="text-center py-8 bg-slate-50 border border-slate-200 rounded-2xl">
                <Lucide.MapPin size={24} className="mx-auto mb-2 text-slate-400" />
                <p className="text-xs font-medium text-slate-600 mb-4">You have no saved addresses.</p>
                <button
                  type="button"
                  onClick={() => {
          ...
          <div className="bg-white border border-slate-200 rounded-3xl p-6 space-y-4 shadow-sm">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              >
                ...
                <span className="text-[10px] text-slate-500 mt-1">Delivered in 20-35 mins ($3.99 fee)</span>
              </button>
              ...
              >
                ...
                <span className="text-[10px] text-slate-500 mt-1">Select a specific slot ($1.99 fee)</span>
              </button>
            </div>
          ...
          <div className="bg-white border border-slate-200 rounded-3xl p-6 space-y-6 shadow-sm">
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {user.savedPaymentMethods.map((card) => (
                    ...
                    >
                      <Lucide.CreditCard size={18} className="text-brand-500 shrink-0" />
                      <div>
                        <p className="text-xs font-bold">{card.cardType} •••• {card.cardNumberLast4}</p>
                        <p className="text-[10px] text-slate-500">Expires {card.expiryDate}</p>
                      </div>
                    </button>
                  ))}
                  ...
                  >
                    <Lucide.PlusCircle size={18} className="text-slate-500 shrink-0" />
                    <div>
                      <p className="text-xs font-bold">Use a new credit card</p>
                      <p className="text-[10px] text-slate-500">Enter card details below</p>
                    </div>
                  </button>
                </div>
              </div>
            ...
            {(!useSavedCard) && (
              <div className="space-y-4 border-t border-slate-200 pt-4 animate-in fade-in duration-200">
                <h4 className="text-xs font-bold text-slate-800">Credit Card Details</h4>
                ...
                <div className="flex items-center space-x-2 pt-2">
                  ...
                  />
                  <label htmlFor="save_card" className="text-xs text-slate-600 font-medium select-none cursor-pointer">
            ...
            {paymentMethodType === 'cod' && (
              <div className="p-4 rounded-2xl bg-emerald-50 border border-emerald-200 text-emerald-700 text-xs flex items-center space-x-3 text-left animate-in fade-in duration-200 shadow-sm">
                ...
                <div>
                  <strong>Pay at door:</strong> Please have the exact amount or let the courier know if you need change.
                </div>
              </div>
            )}
          </div>
        ...
        <div className="bg-slate-50 border border-slate-200 rounded-3xl p-6 space-y-6 shadow-sm">
          ...
          <div className="space-y-3 max-h-48 overflow-y-auto pr-1 custom-scrollbar">
            {cart.map((c, idx) => {
              const extrasPrice = (c.customizations?.extras?.length || 0) * 1.50;
              ...
              return (
                <div key={idx} className="flex justify-between items-center text-xs">
                  ...
                  <span className="font-bold text-slate-900">${totalItemPrice.toFixed(2)}</span>
                </div>
              );
            })}
          </div>
          ...
          <div className="space-y-2">
            <label className="block text-[10px] font-bold text-slate-600 uppercase tracking-wider">Promo Code</label>
            {!appliedCoupon ? (
              ...
              <div className="flex items-center justify-between bg-brand-50 border border-brand-200 p-2.5 rounded-xl">
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-xs font-bold font-mono text-brand-600">{appliedCoupon.code} Applied</span>
                </div>
                <button type="button" onClick={removeCoupon} className="p-1 text-slate-400 hover:text-rose-500 transition-colors">
                  <Lucide.X size={14} />
                </button>
              </div>
            )}
          ...
          <div className="space-y-2 text-xs text-slate-500 font-medium">
            ...
            <div className="flex justify-between text-sm text-[#1A1A1A] font-black">
              ...
              <span className="text-brand-500 drop-shadow-sm">${grandTotal.toFixed(2)}</span>
            </div>
          </div>
